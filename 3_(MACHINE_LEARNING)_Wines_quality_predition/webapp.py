...
def draw_boxplot(df, features, target):
	...
	# for feature, point in zip(features,points):
		if nrows != 1:
			...
			# sns.stripplot(x = target,y = point,color = 'red',alpha = 0.3, ax=axes[row,i])
			i += 1
			...
			# sns.stripplot(x = target,y = point,color = 'red',alpha = 0.3,ax=axes[i])
			i += 1
...
if st.button('Predict quality'):
	...
	plot = draw_boxplot(df,columns,'quality')
